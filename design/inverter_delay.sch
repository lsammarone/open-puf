...
N 10 -90 10 30 { lab=in}
N 10 30 40 30 { lab=in}
...
tran 1p 400p
** plot in out
wrdata ~/open-puf/sims/inverter_delay\{$&run\}.txt v(in) v(out)
reset
let run = run + 1
end
...
.endc
"}
C {devices/lab_pin.sym} -10 -50 0 0 {name=l4 sig_type=std_logic lab=in}
...
C {devices/vsource.sym} -260 20 0 0 {name=V4 value=PULSE"(1.8 0 50ps 10ps 10ps 150ps 1ns)"
}
C {sky130_stdcells/inv_1.sym} -160 -50 0 0 {name=x1 VGND=0 VNB=0 VPB=VDD VPWR=VDD prefix=sky130_fd_sc_hd__ }
